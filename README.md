...
*   **Optimized Resource/Data Pack Loading:** Improves the loading speed of Resource Packs and Data Packs. The mod builds a compact index for compressed packs, directory packs, Forge mod resources, and vanilla resources on first access. This replaces repeated ZIP scans, NIO subtree walks, and filesystem stat calls during namespace discovery, individual lookups, and recursive resource listing.
...
*   **Shipwreck Fix:** When `structure_locate_speedup` is enabled, Shipwreck Fix's four overlapping structure-location mixins are automatically disabled. Its mixins are left untouched when the RO feature is disabled.
...
*   **lazyyyyy:** When `optimize_pack_lookup` is enabled, lazyyyyyy's overlapping vanilla, ZIP, path, and Forge-path pack cache mixins are automatically disabled. Its caches for custom pack types remain enabled.
*   **Boat Break Fix:** When `fix_boat_fall_damage` is enabled, Boat Break Fix's overlapping boat mixin is automatically disabled. It is left untouched when the RO feature is disabled.
