...
/**
 * Immutable in-memory lookup table for path-backed pack resources.
 */
public final class PackResourcesIndex {
    ...
    public static final class Builder {
        ...
        private Builder() {
            ...
            for (int i = 0; i < packTypes.length; ++i) {
                this.builders[i] = new TypeIndexBuilder();
            }
        }

        public void addNamespace(PackType packType, String namespace) {
            if (isLowercase(namespace)) {
                this.addNamespaceUnchecked(packType, namespace);
            }
        }

        void addNamespaceUnchecked(PackType packType, String namespace) {
            this.builders[packType.ordinal()].namespaces.add(namespace);
        }

        /**
         * Adds a resource if no higher-priority root has already supplied it.
         */
        public void addResource(
            ...
            if (typeBuilder.resourcesByLocation.putIfAbsent(
                    location,
                    supplier
            ) != null) {
                return;
            ...
            typeBuilder.resourcesByNamespace
                    ...
                    .add(
                            resourcePath,
                            new IndexedResource(location, supplier)
                    );
        }
    ...
    private static final class TypeIndexBuilder {
        ...
        private TypeIndex build() {
            Map<String, NamespaceIndex> namespaceIndexes =
                    new HashMap<>(capacityFor(
                            this.resourcesByNamespace.size()
                    ));
    ...
    private static final class NamespaceIndexBuilder {
        private final Map<String, List<IndexedResource>>
                resourcesByDirectory = new HashMap<>();
        ...
        private NamespaceIndex build() {
            Map<String, IndexedResource[]> directoryIndexes =
                    new HashMap<>(capacityFor(
                            this.resourcesByDirectory.size()
                    ));
